...
public class AskSolRepository {
    ...
    public List<AskSolEntity> unsolvedAll(){
        return em.createQuery("select askSol from ask_sol askSol where ask_sol.isAnswered = 0", AskSolEntity.class)
                .getResultList();
    }

    public List<AskSolEntity> solvedAll(){
        return em.createQuery("select askSol from ask_sol askSol where ask_sol.isAnswered = 1", AskSolEntity.class)
                .getResultList();
    }

    public AskSolEntity findByBno(long bno){
        return em.createQuery("select askSol from ask_sol askSol where ask_sol.bno = :bno", AskSolEntity.class)
